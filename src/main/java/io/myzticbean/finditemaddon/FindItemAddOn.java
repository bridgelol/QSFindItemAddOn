...
import io.myzticbean.finditemaddon.shop.ShopMenu;
...
//        }
    ...
    private void initFindItemCmd() {
        ...
        try {
            CommandManager.createCoreCommand(this, "finditem", "Search for items from all shops using an interactive GUI", "/finditem", (commandSender, subCommandList) -> {
    ...
    private void initFindItemAdminCmd() {
        ...
        try {
            CommandManager.createCoreCommand(this, "finditemadmin", "Admin command for Shop Search addon", "/finditemadmin", (commandSender, subCommandList) -> {
                if ((commandSender.isOp()) || (!commandSender.isOp() && (commandSender.hasPermission(PlayerPerms.FINDITEM_ADMIN.value()) || commandSender.hasPermission(PlayerPerms.FINDITEM_RELOAD.value())))) {
                    commandSender.sendMessage(ColorTranslator.translateColorCodes(""));
                    commandSender.sendMessage(ColorTranslator.translateColorCodes("&7-----------------------------"));
    ...
    private void initShopMenuCmd() {
        final FileConfiguration config = ConfigSetup.get();

        if (!config.getBoolean("shop-gui.enabled", false)) {
            return;
        }

        final List<String> aliases = config.getStringList("shop-gui.aliases");
        final String mainAlias = aliases.stream().findFirst().orElse("shop");
        final ShopMenu menu = new ShopMenu();

        try {
            CommandManager.createCoreCommand(this, mainAlias, "Shop Browse GUI",
                    "/" + mainAlias,
                    ((commandSender, list) -> {
